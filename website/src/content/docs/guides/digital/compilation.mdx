...
description: How Bloqade lowers, interprets, and emits code — plus the QASM2 dialect groups.
---
...
Bloqade provides a set of [dialects](/guides/digital/dialects/) for QASM2 and our custom extensions to model parallel gates in neutral atom architectures. The basic QASM2 functionality can be enabled via
...
where `structural_no_opt` is the base dialect group (defined in [`kirin`](https://queracomputing.github.io/kirin/latest/)) that provides the basic control flow, common Python expressions (but not all), then:
