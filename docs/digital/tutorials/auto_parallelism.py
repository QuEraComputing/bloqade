...
def build_circuit4():
    ...
    for row in range(L):
        for col in range(L):
            moments.append(cirq.Moment([cirq.CZ(qubits[row, col], qubits[row, (col+1)%L])]))

    # Add vertical nearest-neighbor interactions in separate moments
    for row in range(L):
        for col in range(L):
            moments.append(cirq.Moment([cirq.CZ(qubits[row, col], qubits[(row+1)%L, col])]))
...
def visualize_grid_interactions(circuit, L=4):
    ...
    if num_moments <= len(high_contrast_colors):
        ...
        colors = [high_contrast_colors[i % len(high_contrast_colors)] for i in range(num_moments)]
    ...
    for moment_idx, moment in enumerate(circuit):
        for op in moment:
            if len(op.qubits) == 2:
                ...
                if is_nearest_neighbor(r1, c1, r2, c2, L):
                    ...
                    ax.plot([c1, c2], [r1, r2], '-', 
                           color=colors[moment_idx], linewidth=4, alpha=0.9, zorder=1)
                    ...
                    if r1 == r2:
                        ...
                        con = ConnectionPatch(start, end, "data", "data",
                                             ...
                                             mutation_scale=20, fc=colors[moment_idx],
                    ...
                    con = ConnectionPatch((c1, r1), (c2, r2), "data", "data",
                                         arrowstyle="-", shrinkA=0, shrinkB=0,
                                         mutation_scale=20, fc=colors[moment_idx],
        ...
        legend_elements.append(mpatches.Patch(color=colors[moment_idx], 
                                             label=f'Moment {moment_idx}'))
    
    # Draw grid points (qubits) - extra large size for best visibility
    for row in range(L):
        for col in range(L):
            ax.plot(col, row, 'ko', markersize=75, zorder=3)  # Outer black circle (50% larger)
            ax.plot(col, row, 'wo', markersize=68, zorder=4)  # Inner white circle (50% larger)
            ax.text(col, row, f'{row},{col}', ha='center', va='center', 
                   fontsize=20, color='black', weight='bold', zorder=5)  # Proportionally larger font
    ...
    ax.set_title(f'Qubit Interactions on {L}x{L} Grid by Moment\n'
